...
<table width="778" border="0" align="center" cellspacing="0" cellpadding="0">
    ...
    <td width="602" valign="top" bgcolor="#FFFFFF"><table width="99%" height="487"  border="0" cellpadding="0" cellspacing="0" align="right">
      <tr>
        <td height="24" bgcolor="#EEEEEE" class="tableBorder_thin"><table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td width="40%" class="word_grey">&nbsp;当前位置：<span class="word_darkGrey">培训课程学习  &gt;&gt;作业管理</span></td>
  ...
  <tr>
      ...
      <table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bordercolordark="#FFFFFF" bordercolorlight="#67A8DB">		 
		 <tr>
			 <td colspan="6" height="30"><button onclick="window.location.href='/OnlineTrain/student/course.do?action=coursesMan&srow=0&upid=<%=ctemp.getId() %>'">查看课程文档</button>&nbsp;&nbsp;&nbsp;<img src="../Images/add.gif" width="19" height="18">上传作业
							<input type="FILE" id="file1" Name="file1" size="30"/>&nbsp;&nbsp;<input type="button" value="上传" onclick="modi()"/>
							<br/>培训课程作业提交列表							
			 </td>
		 </tr>
		 <tr> 
			...
			<td align="center" width="9%" bgcolor="#B2D6F1">得分</td>
			<td align="center" width="9%" bgcolor="#B2D6F1">删除</td>		  
		  </tr>	
		  <%
		  ...
		  <tr>
		    ...
		    <td align="center"><a href="download.jsp?url=<%=tf.getUrl() %>"><%=tf.getName() %></a></td>
		    <td align="center"><%=tf.getTeacher() %></td>
		    <td align="center" ><%=tf.getTime() %></td>	
		    <td align="center"><%=tf.getMark() %></td>
			<td align="center"><%if(0==tf.getMark()){ %><a onclick="return check();" href="deletefile.jsp?url=<%=tf.getUrl() %>&upid=<%=tf.getId() %>">删除 </a><%}else{ %>删除<%} %></td>			
		  </tr><%
		  	i++;
		 }
	...
	<html:link page="/student/course.do?action=tasksMan&srow=0">首页</html:link>&nbsp;<%
		if((srow-10)>=0){
		  newValues.put("srow",srow-10);  		  
		  pageContext.setAttribute("newValues",newValues);
  %><html:link page="/student/course.do?action=tasksMan" name="newValues">上一页</html:link>&nbsp;
	 <%}else{ %>上一页&nbsp;<%}
	   if((srow+10)<=allrow){
	   	  newValues.put("srow",srow+10);
		  pageContext.setAttribute("newValues",newValues); 
	  %><html:link page="/student/course.do?action=tasksMan" name="newValues">下一页</html:link>&nbsp;
		 <%}else{%>下一页&nbsp;<%} newValues.put("srow",(allrow/10)*10);pageContext.setAttribute("newValues",newValues);
	 %><html:link page="/student/course.do?action=tasksMan" name="newValues">尾页</html:link>&nbsp;	 
	</td>
	</tr>	 
	</table>
	</form>
