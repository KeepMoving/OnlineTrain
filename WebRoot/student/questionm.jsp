...
<%	
	List lists = (List) session.getAttribute("moreleQue");	
	Iterator it = lists.iterator();	
	if(it.hasNext()){
 %>	
	<html:form action="/manage/questions.do?action=submitMore" method="post">
	<table id="m" width="580"  border="0" cellspacing="0" cellpadding="0" align=center>
	  <tr>
	    <td colspan="4" align="left" height="23" style="font-size:11pt;">&nbsp;&nbsp;多项选择试题练习</td>
	  </tr>	 
	   <%	   	while (it.hasNext()) {
	  ...
	  <tr>
	    <td width="60" height=23 nowrap>&nbsp;</td>
	    <td width="20" align=righit nowrap><input type="checkbox" name="answerArrM<%=i %>" border="no" value="D"/></td>
	    <td width="20" align=center nowrap>D.</td>
	    <td width="480" align=left nowrap><%=temps.getOptionD() %></td>
	  </tr>	
<logic:present name="morAns" scope="request"> 
		<%	
		QuestionsForm answerSin = (QuestionsForm)request.getAttribute("morAns");
		 ans = answerSin.getAnswerArrM();	
		 %>
	  <tr>
 ...
 <% i++;} 	  
	  ...
	  <%   String tsrow =(String)request.getAttribute("srow").toString();
		 ...
		 newValues.put("type",2); 		 
	%>
	  	<td colspan="4" align="center">
	  	 <input type="hidden" name="optionA" value="2" />
	...
	<html:link page="/manage/questions.do?action=selectList&srow=0&type=2">首页</html:link>&nbsp;<%
		if((srow-5)>=0){				 
