...
<%@ page import="com.wgh.dao.CourDeDAO"%>
...
<%	
	...
	try{
		File file = new File(path+"\\upload\\courdel\\"+fname);
		if(file.exists()){
			mySmartUpload.downloadFile(url);
		}else{
			CourDeDAO cddao = new CourDeDAO();
			int flag=cddao.delByUrl(url);
			if(flag!=1){
		...
		%>
			...
			alert("文件已经不存在!");
			window.location.href="/OnlineTrain/student/course.do?action=coursesMan&srow=0";
			</script>	
		<%
		}			
	}catch(Exception e){
