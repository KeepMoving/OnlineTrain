...
public class PaResultDAO {
	...
	public int insert(PaResultForm prf){		
        ...
        int falg = 0;
            try {
            	sql = "INSERT INTO paresult (paid,patitle,userid,sscore,mscore,subtime,course) values(" +prf.getPaid()+",'"+prf.getPatitle()+"','"+prf.getUserid()+"',"
            	+prf.getSscore()+","+prf.getMscore()+",'"+prf.getSubtime()+"','"+prf.getCourse()+"')";                                     
	...
	public List query(String stuId) {
        ...
        try {
            while (rs.next()) {
                ...
                stuResultForm1.setCourse(rs.getString(8));
